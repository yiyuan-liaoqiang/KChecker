...
#define kDMLazyScrollViewTransitionDuration     0.3
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {
    if (otherGestureRecognizer.view == self.tapEffectiveView && self.tapEffectiveView) {
        return YES;
    }
    return NO;
}
...
- (void) setNumberOfPages:(NSUInteger)pages {
    if (pages != numberOfPages) {
        numberOfPages = pages;
        NSUInteger offset = [self hasMultiplePages] ? numberOfPages + 2 : 1;
...
- (UIViewController *) loadControllerAtIndex:(NSInteger) index andPlaceAtIndex:(NSInteger) destIndex {
    UIViewController *viewCtl = dataSource(index);
    viewCtl.view.tag = 0;
