...
@implementation ECQRCodeScanView

- (instancetype)initWithFrame:(CGRect)frame superViewLayer:(CALayer *)superViewLayer {
    if (self = [super initWithFrame:frame]) {
        self.superLayer = superViewLayer;
        // 创建扫描边框
        [self buildUI];
    }
    return self;
...
- (void)buildUI {
    ...
    [self.superLayer addSublayer:scanContentView.layer];
    ...
    [self.superLayer addSublayer:_animation_line.layer];
...
#pragma mark - - - 扫描外部View的创建
    ...
    UIImage *img = [UIImage imageNamed:@"saoyisaoIconShoudian"];
    CGFloat light_buttonX = (self.bounds.size.width - img.size.width) / 2;
    CGFloat light_buttonY = CGRectGetMaxY(promptLabel.frame) + scanContent_X * 0.5;
    light_button.frame = (CGRect){light_buttonX, light_buttonY, img.size};
    [light_button setImage:img forState:UIControlStateNormal];
    [light_button setImage:img forState:UIControlStateHighlighted];
    [light_button addTarget:self action:@selector(light_buttonClick:) forControlEvents:UIControlEventTouchUpInside];
    [bottom_View addSubview:light_button];
    
    UILabel *light_label = [[UILabel alloc] init];
    light_label.frame = (CGRect){0,CGRectGetMaxY(light_button.frame) + 30.0f,self.bounds.size.width,17.5f};
    light_label.textAlignment = NSTextAlignmentCenter;
    light_label.textColor = [UIColor whiteColor];
    [bottom_View addSubview:light_label];
...
- (void)light_buttonClick:(UIButton *)button {
