...
class ReplaceFormInputController: BaseFormViewController {
    override func viewDidLoad() {
        
        let filePath = CommonUtils.bundlePathWithFileName(fileName: "replaceFormInput.plist")
    ...
    func getData() {
        if let contentId = self.dataModel?.value(forKey: "contentId") as? String {
            let url = "http://111.229.39.85:9094/replace/" + contentId
                ...
                YYNSessionManager.default().method("get", urlString: url, andParams: [:], andHttpHeaders: [:], success: { (ret) in
                    ...
                    ActivityIndicatorManager.hideActivityIndicator(in: self.view)
                        ...
                        let sql = "select * from t_plan_check where facilityId = '" + facilityId + "'" + "and versionType = 'v2_replace'"
    ...
    @objc func publish(){
        ...
        ActivityIndicatorManager.showActivityIndicator(in: self.view)
        session?.method("post", urlString: "http://111.229.39.85:9094/facility/replace", andParams: param, andHttpHeaders: [:], success: { (ret) in
            ActivityIndicatorManager.hideActivityIndicator(in: self.view)
            if let ret = ret as? [String:Any] {
            ...
            if let error = err as? NSError {
                if error.code == -1009 || error.code == -1004 {
                    ...
                    localRequest?["http://111.229.39.85:9094/facility/replace"] = JSON(param ?? [:])
