...
class LubricationInputViewController: BaseFormViewController {
    override func viewDidLoad() {
        
        let filePath = CommonUtils.bundlePathWithFileName(fileName: "lubrication.plist")
    ...
    func getData() {
        if let contentId = self.dataModel?.value(forKey: "contentId") as? String {
            let url = "http://111.229.39.85:9094/lubrication/" + contentId
    ...
    @objc func publish(){
        ...
        ActivityIndicatorManager.showActivityIndicator(in: self.view)
        session?.method("post", urlString: "http://111.229.39.85:9094/facility/lubrication", andParams: param, andHttpHeaders: [:], success: { (ret) in
            ActivityIndicatorManager.hideActivityIndicator(in: self.view)
            if let ret = ret as? [String:Any] {
            ...
            if let error = err as? NSError {
                if error.code == -1009 || error.code == -1004 {
                    ...
                    localRequest?["http://111.229.39.85:9094/facility/lubrication"] = JSON(param ?? [:])
